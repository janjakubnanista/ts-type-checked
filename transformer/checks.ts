...
export const isFunctionType = (typeChecker: ts.TypeChecker, type: ts.Type, root?: ts.TypeNode): boolean => {
  const typeNode = typeChecker.typeToTypeNode(type, root);
  if (typeNode?.kind === ts.SyntaxKind.FunctionType) return true;
...
export const isTupleType = (typeChecker: ts.TypeChecker, type: ts.Type, root?: ts.TypeNode): boolean => {
  const typeNode = typeChecker.typeToTypeNode(type, root);
  return typeNode?.kind === ts.SyntaxKind.TupleType;
};
